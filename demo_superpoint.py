...
# --- 1. HELPER CLASSES (RENAMED TO MATCH YOUR WEIGHTS) ---

class DoubleConv(nn.Module):
    """(convolution => [BN] => ReLU) * 2"""
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # Renamed from 'double_conv' to 'conv' to match your weights
...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # Renamed from 'maxpool_conv' to 'mpconv' to match 'down1.mpconv'
...
# --- 2. MAIN NETWORK CLASS ---

class SuperPointNet(nn.Module):
    """
    Custom SuperPoint Architecture with Batch Norm.
    Replaces the default PySLAM SuperPointNet.
    """
    def __init__(self):
        super(SuperPointNet, self).__init__()
        ...
        # Detector Head (Explicit BN layers based on your 'bnPa' keys)
...
# --- 3. FRONTEND WRAPPER (REQUIRED BY PYSLAM) ---

class SuperPointFrontend(object):
    """ Wrapper around SuperPointNet to handle input/output processing """
    ...
    def nms_fast(self, in_corners, H, W, dist_thresh):
        ...
        if rcorners.shape[1] == 0:
            return np.zeros((3,0)).astype(int), np.zeros((H, W)).astype(int)
        if rcorners.shape[1] == 1:
            out = np.vstack((rcorners, in_corners[2])).reshape(3,1)
            return out, np.zeros((H, W)).astype(int)
        ...
        # Get all surviving -1's and return sorted results
    ...
    def run(self, img):
        """
        Process a numpy image (H, W) or (H, W, C)
        """
        import numpy as np 
        import cv2
